var app = (function (exports) {
  ...
  define$1(Rgb$1, rgb$1, extend$3(Color$1, {
    brighter: function(k) {
  ...
  define$1(Hsl$1, hsl$1, extend$3(Color$1, {
    brighter: function(k) {
      k = k == null ? brighter$1 : Math.pow(brighter$1, k);
  ...
  function buildInterpolator$1(stops, base = 1) {
    if (!stops || stops.length < 2 || stops[0].length !== 2) return;

    // Confirm stops are all the same type, and convert colors to arrays
    const type = getType$1(stops[0][1]);
    if (!stops.every(s => getType$1(s[1]) === type)) return;
    stops = stops.map(([x, y]) => [x, convertIfColor$1(y)]);

    const izm = stops.length - 1;

    const scale = getScale$1(base);
    const interpolate = getInterpolator$1(type);
  ...
  function getType$1(v) {
    return color$1(v) ? "color" : typeof v;
  }

  function convertIfColor$1(val) {
    // Convert CSS color strings to clamped RGBA arrays for WebGL
    if (!color$1(val)) return val;
    const c = rgb$1(val);
    return [c.r / 255, c.g / 255, c.b / 255, c.opacity];
  }

  function getScale$1(base) {
  ...
  function getInterpolator$1(type) {
  ...
  function autoGetters$1(properties = {}, defaults) {
    return Object.entries(defaults).reduce((d, [key, val]) => {
      d[key] = buildStyleFunc$1(properties[key], val);
      return d;
    }, {});
  }

  function buildStyleFunc$1(style, defaultVal) {
    if (style === undefined) {
      return getConstFunc$1(defaultVal);

    } else if (typeof style !== "object" || Array.isArray(style)) {
      return getConstFunc$1(style);

    } else {
      return getStyleFunc$1(style);

    } // NOT IMPLEMENTED: zoom-and-property functions
  }

  function getConstFunc$1(rawVal) {
    const val = convertIfColor$1(rawVal);
    const func = () => val;
    return Object.assign(func, { type: "constant" });
  }

  function getStyleFunc$1(style) {
    ...
    const getVal = (type === "identity")
      ? convertIfColor$1
      : buildInterpolator$1(stops, base);
  ...
  function getStyleFuncs$1(inputLayer) {
    const layer = Object.assign({}, inputLayer); // Leave input unchanged

    // Replace rendering properties with functions
    layer.layout = autoGetters$1(layer.layout, layoutDefaults$1[layer.type]);
    layer.paint  = autoGetters$1(layer.paint,  paintDefaults$1[layer.type] );
...
`;
  ...
  var utf8TextDecoder$1 = new TextDecoder('utf-8');
  ...
  function writePackedVarint$1(arr, pbf)   { arr.forEach(pbf.writeVarint, pbf);   }
  function writePackedSVarint$1(arr, pbf)  { arr.forEach(pbf.writeSVarint, pbf);  }
  function writePackedFloat$1(arr, pbf)    { arr.forEach(pbf.writeFloat, pbf);    }
  function writePackedDouble$1(arr, pbf)   { arr.forEach(pbf.writeDouble, pbf);   }
  function writePackedBoolean$1(arr, pbf)  { arr.forEach(pbf.writeBoolean, pbf);  }
  function writePackedFixed32$1(arr, pbf)  { arr.forEach(pbf.writeFixed32, pbf);  }
  function writePackedSFixed32$1(arr, pbf) { arr.forEach(pbf.writeSFixed32, pbf); }
  function writePackedFixed64$1(arr, pbf)  { arr.forEach(pbf.writeFixed64, pbf);  }
  function writePackedSFixed64$1(arr, pbf) { arr.forEach(pbf.writeSFixed64, pbf); }
  ...
  function init(userParams) {
    const { glyphEndpoint, styles } = setParams$1(userParams);
...
`;
  ...
  function setup(error, data, style) {
    ...
    const mixer = init({ glyphs, layers });
  ...
  function render(data, style) {
    ...
    resizeCanvasToDisplaySize(canvas, pixRatio);
